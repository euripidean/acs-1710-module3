...
def compliments_results():
    ...
    user_compliments = []
    number = (request.args.get('num_compliments'))
    compliments = request.args.get('wants_compliments')
    if compliments == 'yes':
        number = int(number)
        user_compliments = random.sample(list_of_compliments,number)
    context = {
        "users_name": request.args.get('users_name'),
        "compliments": user_compliments,
    }
...
def animal_facts():
    ...
    choice = request.args.get('animals')
    if choice is not None:
        choice = int(choice)
        animal = animal_list[choice]
...
def image_filter():
    ...
    if request.method == 'POST':
        ...
        #save image and return new file path
